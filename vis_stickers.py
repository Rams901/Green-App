...
def draw_bounding_box_on_image_array(image,
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  draw_bounding_box_on_image(image_pil, ymin, xmin, ymax, xmax, sticker_name, color,
...
def draw_bounding_box_on_image(image,
  ...
  sticker = Image.open("C:/Users/r901/Desktop/newfolder/Face_Recognition/src/"+sticker_name).convert("RGBA")
  #sticker.thumbnail(400, 200)
  size = ((int(right)-int(left))//2-90, (int(bottom)-int(top))//2)
  ...
  x, y = sticker.size 
  size = ((int(right)+int(left))//2, (int(bottom)+int(top)) //2)
  w, z = size
  try:
   image.paste(sticker, (w, z- 25, w+x, z-25+y), sticker)
  except ValueError:
          sticker.thumbnail((x // 2, y // 2))
          x, y = sticker.size
          image.paste(sticker, (w, z-25, w+x, z-25+ y), sticker)
...
########## TRY DELETING THIS ONE!!! FOR SHWOING CONFIDENCE LEVEL:!!!!!!!!#########
...
def draw_bounding_boxes_on_image_array(image,
  ...
  draw_bounding_boxes_on_image(image_pil, boxes, sticker_name, color, thickness,
                               display_str_list_list)
  np.copyto(image, np.array(image_pil))
...
def draw_bounding_boxes_on_image(image,
  ...
  for i in range(boxes_shape[0]):
    ...
    draw_bounding_box_on_image(image, boxes[i, 0], boxes[i, 1], boxes[i, 2],
                               boxes[i, 3], sticker_name, color, thickness, display_str_list)
...
def draw_keypoints_on_image_array(image,
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  draw_keypoints_on_image(image_pil, keypoints, sticker_name,  color, radius,
                          use_normalized_coordinates)
  np.copyto(image, np.array(image_pil))
...
def visualize_boxes_and_labels_on_image_array(
  ...
  for box, color in box_to_color_map.items():
    ...
    if instance_boundaries is not None and ():
      draw_mask_on_image_array(
          image,
